...
- Spring AI의 기본 `JdbcChatMemoryRepository`는 `SPRING_AI_CHAT_MEMORY` 테이블을 사용하지만, 우리는 `CHAT_MESSAGE` 테이블에 `message_id`(UUID)를 포함한 자체 스키마가 필요했음
