 ...
 * ModelBlockConfig를 통해 동적으로 블록을 제거합니다.
 * </p>
 */
public final class BlockProcessor {
     ...
     * @return 정제된 텍스트
     */
    public static String stripBlocks(String rawContent, ModelBlockConfig config) {
     ...
     * @param config 모델별 블록 설정
     */
    public static void removeCompleteBlocks(StringBuilder buffer, ModelBlockConfig config) {
        Pattern pattern = config.getBlockPattern();
        while (true) {
     ...
     * 버퍼에서 전송 가능한 텍스트 추출
     * <p>
     * 블록 시작 태그 이전까지의 텍스트만 추출하여 반환합니다.
     * 스트리밍 중 블록 내용이 클라이언트에게 전송되지 않도록 합니다.
     * </p>
     *
     * @param buffer 텍스트 버퍼
     * @param config 모델별 블록 설정
     * @return 전송 가능한 텍스트
     */
    public static String drainSendableText(StringBuilder buffer, ModelBlockConfig config) {
        if (buffer.length() == 0) {
            return "";
        }
